...
def npz_to_submission(filename):
    ...
    df.to_csv('./results/siglip2gaint384_single_baseline.csv', index=False)

    save_path = filename.replace('.npz', '.csv')
    
    print(f'✅ csv saved in kaggle submission format: {save_path}')

filename1 = "./results/siglip2gaint384_multi_baseline.npz"
filename2 = "./results/siglip2gaint384_single_baseline.npz"
