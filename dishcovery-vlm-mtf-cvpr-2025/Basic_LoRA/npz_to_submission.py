...
def npz_to_submission(filename):
    ...
    df.to_csv('./results/sigliplarge384_single_lora1.csv', index=False)

    save_path = filename.replace('.npz', '.csv')
    
    print(f'✅ csv saved in kaggle submission format: {save_path}')

filename1 = "./results/sigliplarge384_multi_lora1.npz"
filename2 = "./results/sigliplarge384_single_lora1.npz"
